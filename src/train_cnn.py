...
def train_one_scenario(scenario: dict, cfg: TrainConfig, device: torch.device):
    print(f"[CNN][{scenario['name']}] Starting training...")
    ...
    for epoch in range(cfg.epochs):
        ...
        print(
            f"[CNN][{scenario['name']}] Epoch {epoch + 1}/{cfg.epochs} "
            f"loss={avg_loss:.4f} val_acc={val_acc:.4f}"
        )
        if val_acc > best_val_acc:
            best_val_acc = val_acc
            best_state = {k: v.detach().cpu().clone() for k, v in model.state_dict().items()}
            print(f"[CNN][{scenario['name']}] New best val_acc={best_val_acc:.4f}")

    assert best_state is not None
    model.load_state_dict(best_state)

    cm_path = os.path.join(IMAGE_DIR, "confusion_matrices", f"cnn_{scenario['name']}_test_cm.png")
    test_acc, per_class_acc = evaluate(
        ...
        cfg.batch_size,
        cm_path=cm_path,
        title=f"CNN Test - {scenario['name']}",
    )

    os.makedirs(MODEL_DIR, exist_ok=True)
    model_path = os.path.join(MODEL_DIR, f"cnn_{scenario['name']}.pt")
    torch.save(
        {
