...
def loss_curve_path_for(
    ...
    return os.path.join(
        ...
        "loss_curves",
        f"cnn_{pair_name}_{artifact_tag(run_index, artifact_label)}_loss_curve.png",
    )
...
def combine_dataset_loss_curves(
    ...
    draw.text(
        (padding, 15),
        f"CNN {dataset_cfg.title_label} Loss Curves - {artifact_display_name(run_index, artifact_label)}",
        fill="black",
    )
    ...
    out_path = os.path.join(
        dataset_cfg.image_dir,
        f"cnn_{dataset_cfg.name}_loss_curves_{artifact_tag(run_index, artifact_label)}_grid.png",
    )
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
...
def train_one_pair(
    ...
    print(f"[CNN][{dataset_cfg.name}][{pair_name}][{display_name}] Starting training...")
    ...
    for epoch in range(cfg.epochs):
        ...
        for batch_idx, (x, y, _class_name) in enumerate(train_dl, start=1):
            ...
            if batch_idx == 1 or batch_idx % progress_interval == 0 or batch_idx == len(train_dl):
                print(
                    f"[CNN][{dataset_cfg.name}][{pair_name}][{display_name}] "
        ...
        print(
            f"[CNN][{dataset_cfg.name}][{pair_name}][{display_name}] Epoch {epoch + 1}/{cfg.epochs} "
    ...
    save_loss_curve(
        history,
        loss_curve_path,
        title=f"CNN {dataset_cfg.title_label} Loss Curve - {pair_name} ({display_name})",
    )
    ...
    model_path = os.path.join(
        ...
        f"cnn_{pair_name}_{artifact_tag(run_index, artifact_label)}.pt",
    )
    torch.save(
        {
